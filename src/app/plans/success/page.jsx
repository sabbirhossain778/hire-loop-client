...
export default async function Success({ searchParams }) {
        ...
        return (
            <div className="w-full min-h-screen bg-zinc-950 text-zinc-50 flex flex-col justify-center items-center p-6 select-none">
                <section className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center">
                    <div className="w-16 h-16 bg-red-500/10 text-red-400 rounded-full flex items-center justify-center mx-auto mb-6 text-2xl">
                        ❌
                    </div>
                    <h1 className="text-xl font-bold text-zinc-50 mb-2">Invalid Access</h1>
                    <p className="text-zinc-400 text-sm mb-6">
                        No active payment session was found. If you just completed a payment, please check your email or return home.
                    </p>
                    <Link href="/" className="block w-full text-center text-xs font-bold px-4 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl transition">
                        Return to Homepage
                    </Link>
                </section>
            </div>
        )
        // নোট: তুমি চাইলে সরাসরি redirect('/')-ও করে দিতে পারো।
    }
    ...
    if (status === 'complete') {
        ...
        return (
            <div className="w-full min-h-screen bg-zinc-950 text-zinc-50 flex flex-col justify-center items-center p-6 select-none relative overflow-hidden">
                {/* bg glow effect */}
                <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-72 h-72 bg-emerald-500/10 rounded-full blur-[100px] pointer-events-none" />

                <section
                    id="success"
                    className="relative max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl text-center z-10"
                >
                    {/* green badge icon */}
                    <div className="w-16 h-16 bg-emerald-500/10 text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-6 border border-emerald-500/20 shadow-[0_0_24px_rgba(16,185,129,0.1)] text-2xl">
                        ✅
                    </div>
                    ...
                    {/* info box */}
                    <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-4 text-left space-y-3 text-xs mb-8">
                        <div>
                            <span className="block font-semibold text-zinc-500 mb-1 uppercase tracking-wider">
                                Confirmation Email
                            </span>
                            <span className="text-zinc-200 break-all text-sm font-medium">
                                📩 {customerEmail}
                            </span>
                        </div>

                        <div className="border-t border-zinc-800/60 pt-3 flex flex-col gap-1 text-zinc-500">
                            <span>Have billing questions or need custom support?</span>
                            <a
                                href="mailto:orders@example.com"
                                className="text-blue-400 hover:text-blue-300 font-medium transition inline-block w-fit"
                            >
                                orders@example.com
                            </a>
                        </div>
                    ...
                    <div className="space-y-3">
                        <Link
                            href="/dashboard"
                            className="block w-full text-center text-xs font-bold px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl shadow-lg shadow-blue-950/30 transition duration-200"
                        >
                            Go to Dashboard
                        </Link>
                        ...
                        <Link
                            href="/"
                            className="inline-block text-xs font-medium text-zinc-400 hover:text-zinc-200 transition pt-1"
                        >
                            ← Return to Homepage
                        </Link>
                    </div>
